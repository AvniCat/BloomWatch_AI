# Elevated Bloom Risk Prediction — Kerala & Karnataka Coasts

Machine-learning pipeline for predicting **elevated bloom risk** — operationally
defined as satellite-derived chlorophyll-a > 2 mg/m³ per 8-day window — along the
southwest coast of India, using satellite, in-situ, and rainfall observations
from 2020–2024.

**Important scope note:** the model forecasts elevated-chlorophyll weeks, not
confirmed toxic Harmful Algal Bloom (HAB) events. See `LIMITATIONS.md` for the
distinction, sample-size caveats, and species-specific blind spots (notably
Trichodesmium blooms, which the standard satellite chlorophyll proxy does not
detect reliably).
